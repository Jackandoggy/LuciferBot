...
from ufsbotz import (BOT_USERNAME, BOT_NAME, MESSAGE_DUMP_CHAT, SUDOERS,
                     ufs, arq)
from ufsbotz.core.keyboard import ikb
from ufsbotz.core.tasks import _get_tasks_text, all_tasks, rm_task
from ufsbotz.core.types import InlineQueryResultCachedDocument
from ufsbotz.modules.info import get_chat_info, get_user_info
from ufsbotz.modules.music import download_youtube_audio
from ufsbotz.utils.functions import test_speedtest
...
async def alive_function(answers):
    ...
    bot_state = "Dead" if not await ufs.get_me() else "Alive"
    buttons.add(
...
async def tg_search_func(answers, text, user_id):
    ...
    if str(text)[-1] != ":":
        ...
        return answers
    text = text[:-1]
    async for message in ufs.search_global(text, limit=49):
        buttons = InlineKeyboard(row_width=2)
        buttons.add(
            InlineKeyboardButton(
...
**Longitude:** `{info['lon']}`
"""
    await ufs.edit_inline_text(inline_message_id, msg)


async def ping_func(answers):
    ping = Ping(ping_id=ufs.rnd_id())
    t1 = time()
    await ufs.send(ping)
    t2 = time()
...
async def info_inline_func(answers, peer):
    ...
    try:
        user = await ufs.get_users(peer)
        caption, _ = await get_user_info(user, True)
    except IndexError:
        try:
            chat = await ufs.get_chat(peer)
...
@ufs.on_callback_query(filters.regex("^cancel_task_"))
async def cancel_task_button(_, query: CallbackQuery):
    ...
    await ufs.edit_inline_text(
        query.inline_message_id,
        text,
    )

    if keyb:
        await ufs.edit_inline_reply_markup(
            query.inline_message_id,
            keyb,
        )
