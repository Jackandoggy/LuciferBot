...
async def imdb(client, message):
    ...
    reply_markup = InlineKeyboardMarkup(
        [[InlineKeyboardButton("More Info", url=f"https://www.imdb.com/title/tt{movie.movieID}")]]
    )
...
<b>Country:</b> <code>{movie['country'] if 'country' in movie else 'Not Found'}</code>\n\
    "
    try:
        m = await message.reply_photo(movie['full-size cover url'], caption=caption, reply_markup=reply_markup)
    except KeyError:
        m = await message.reply_photo(movie['cover url'], caption=caption, reply_markup=reply_markup)
